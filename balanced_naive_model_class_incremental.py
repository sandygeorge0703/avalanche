...
pattern = 'print24|print131|print0|print46|print82|print111|print132|print37'
...
data_filtered.iloc[:, 0] = data_filtered.iloc[:, 0].str.replace(
    r'.*?/(print24|print131|print0|print46|print82|print111|print132|print37)/(image-\d+\.jpg)', 
    r'\1/\2', 
    regex=True
)
...
def filter_and_reindex(data_frame, root_dir):
    ...
    """
    valid_indices = []
    allowed_folders = {"print24", "print131", "print0", "print46","print82","print111","print132", "print37"}
...
for exp_id in [1, 2, 3]:
    ...
    train_sampler = BalancedBatchSampler(data_frame=filtered_train_data, batch_size=15, samples_per_class=15)
    valid_sampler = BalancedBatchSampler(data_frame=filtered_valid_data, batch_size=15, samples_per_class=15)
    test_sampler  = BalancedBatchSampler(data_frame=filtered_test_data,  batch_size=15, samples_per_class=15)
...
for exp_idx, experience in enumerate(benchmark.train_stream):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # --- NEW CODE START: Compute and print confusion matrices ---
        # Training confusion matrix:
        train_cm_benchmark = benchmark_from_datasets(
            train=[current_train_loader.dataset],
            test=[current_train_loader.dataset]
        )
        train_cm_res = cl_strategy.eval(train_cm_benchmark.test_stream)
        print(f"Epoch {epoch} Train Confusion Matrix:")
        print(train_cm_res.get("ConfusionMatrix_Stream/eval_phase/test_stream", "No confusion matrix available"))
        
        # Validation confusion matrix (from val_res):
        print(f"Epoch {epoch} Validation Confusion Matrix:")
        print(val_res.get("ConfusionMatrix_Stream/eval_phase/test_stream", "No confusion matrix available"))
        
        # Test confusion matrix:
        test_cm_benchmark = benchmark_from_datasets(
            train=[current_test_loader.dataset],
            test=[current_test_loader.dataset]
        )
        test_cm_res = cl_strategy.eval(test_cm_benchmark.test_stream)
        print(f"Epoch {epoch} Test Confusion Matrix:")
        print(test_cm_res.get("ConfusionMatrix_Stream/eval_phase/test_stream", "No confusion matrix available"))
        # --- NEW CODE END ---
